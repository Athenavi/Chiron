...
欢迎贡献!Chiron 是一个多租户 SaaS AI Agent 平台(Go 网关 + Python AI 引擎 + Vue 3 前端)。在提交 PR 前，请先阅读本文档与 [SECURITY.md](SECURITY.md)。
...
# 3. 配置环境变量
cp .env.example .env
#    编辑 .env: APP_SECRET(必需, python -c "import secrets; print(secrets.token_urlsafe(32))")
...
- 错误处理:不吞错、不装 `panic`;日志使用 `log/slog` 结构化输出。
...
test(api): /ready 新增契约适配
docs(architecture): 补充多租户隔离矩阵
ci: 三段 CI 流水线(go/pytest/vue-tsc)
```
...
4. CI 三段检查(gateway / engine / frontend)全部通过;
