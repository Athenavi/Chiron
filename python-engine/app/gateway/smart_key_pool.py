...
注意(中间态)：管理端添加的 Key 仅保存在进程内存，重启即丢失，且不再明文落盘 JSON。
持久化/多实例一致性的正式方案见 docs/llm-provider-key-management-dr.md（集中派：
网关加密落库 + Redis keyset/pub-sub 下发），待该 DR 实现后接入。
"""
...
class SmartAPIKeyPool:
    ...
    持久化：本实现为纯内存（不落盘、不落库），由调用方负责启动时注入 env 密钥。
    管理端添加的密钥在 DR（见模块 docstring）落地前仅内存有效。
    """
    ...
    async def add_key(self, provider: str, key: str, remark: str = "") -> None:
        """添加 Key（幂等：同 provider+key 已存在时仅更新备注；仅内存，不落盘）"""
        async with self._lock:
    ...
    async def remove_key_by_id(self, key_id: str) -> bool:
        """按 ID 删除 Key（管理端路径删除，无需请求体携带完整 key；仅内存）"""
        async with self._lock:
