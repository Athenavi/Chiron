...
const (
	// permsCacheKeyPrefix 是有效权限缓存键前缀，完整键�?ent:rbac:perms:{userID}
...
//
// 返回值语义（防越权的关键区分）：
//   - nil, nil：用户没有任�?ent 角色记录（直�?+ 群组均未关联），
//     调用方应回退旧权限体系（auth.HasPermission）。此状态不写缓存�?
//   - �?nil 空切�? nil：用户有角色记录但聚合权限为空，表示"明确无权�?�?
//     调用方禁止回退旧体系。此状态以 "[]" 序列化缓存�?
//   - 非空切片, nil：聚合后的权限点列表（已去重）�?
//
// Redis 不可用或读取失败时降级直�?DB（slog.Warn，不返回错误）；
// DB 查询失败返回 error（由调用方决�?fail-open/fail-close 策略）�?
func LoadEffectivePerms(ctx context.Context, userID string) ([]string, error) {
	...
	if !hasRoles {
		// �?ent 角色记录：不缓存（nil 语义），交由调用方回退旧权限体�?
		return nil, nil
	}

	// ── 3. 回写缓存（含空切片："[]" 保证"明确无权�?语义可缓存）──
	if rdb := db.Redis; rdb != nil {
...
// queryEffectivePerms �?PG 聚合用户的有效权限�?
...
// Redis 不可用或删除失败时仅记录日志（缓存最�?5 分钟自然过期）�?
...
// 供群�?角色写操作（修改群组成员、群组角色绑定、角色权限变更）后调用�?
// 成员查询失败时仅记录日志（依�?TTL 自然过期兜底，不阻断写路径）�?
...
// encodePerms 将非 nil 权限切片序列化为缓存值（空切�?�?"[]"）�?
