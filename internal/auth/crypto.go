...
// EnvOIDCSecretKey 是企�?SSO 加密密钥的环境变量名�?
// 原始值必�?�?32 字节；内部经 SHA-256 归一化为 32 字节 AES-256 密钥�?
// 未配置时 SSO 管理写接口返�?503，读/发现接口不受影响�?
const EnvOIDCSecretKey = "ENT_OIDC_SECRET_KEY"

// LoadOIDCEncryptionKey 从环境变量加�?SSO 加密密钥�?
// 未配置或长度不足 32 字节时返�?nil（调用方据此返回 503 提示配置缺失）�?
...
// EncryptAESGCM 使用 AES-256-GCM 加密明文，返�?base64(nonce || ciphertext || tag)�?
func EncryptAESGCM(key []byte, plaintext string) (string, error) {
	...
	// Seal(nonce, ...) �?nonce 作为前缀拼接送出，解密时按前缀切分
