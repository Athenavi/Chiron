...
// EnterpriseBillingObserver 是把消费事件归集�?billing_records �?
// BillingObserver（企业成本中心）。写入完全异步（goroutine + recover），
// 任何失败�?slog.Warn，绝不阻塞或影响主计费链路�?
//
// 接线位置（集成任务）：gateway_router.go NewGatewayRouter �?
...
// pool �?nil 时事件处理回退到全局 db.Pool（沿用项�?PGStore 模式）�?
func NewEnterpriseBillingObserver(pool *pgxpool.Pool) *EnterpriseBillingObserver {
	return &EnterpriseBillingObserver{pool: pool}
}

// OnCreditChange 异步写一�?billing_records 归集记录�?
// 仅归集消费（扣减）事件；充�?退款等入账�?payments / credit_transactions 覆盖�?
...
// record 执行一次归集写入：tenant_id �?users 表关联，
// group_id 取用户主群组（ent_group_members 第一条），无群组�?NULL�?
func (o *EnterpriseBillingObserver) record(evt CreditEvent) {
	...
	// group_id：用户主群组（ent_group_members 第一条，�?group_id 排序保证确定性）�?
	...
	// 1 credit = 1 分（�?payments.amount_cents 口径一致）
	costCents := -evt.Amount
	// 注意：CreditEvent 不携�?token 明细（Manager 事件仅含金额），
	// input/output_tokens �?0，成本以 cost_cents 为准�?
