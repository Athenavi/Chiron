...
const (
	...
	PayStatusExpired  = "expired"  // 超时未支�?
)

// Payment 是一笔充值订单（支付�?微信/PayPal 共用）�?
// 金额�?�?为单位：支付�?微信为人民币分（CNY），PayPal 为美元分（USD）�?
type Payment struct {
	ID              string     `json:"id"`
	...
	QRCode          string     `json:"qr_code,omitempty"`           // 扫码支付二维码内�?
...
type PaymentStore interface {
	...
	// MarkPaymentPaid 幂等推进 pending→paid；若订单�?pending 返回 (nil, nil) 表示已处理�?
...
//  1. 订单 pending→paid 原子推进（重复回�?轮询只入账一次）�?
//  2. 推进成功后向用户�?credits（reason = "<channel>_topup"）�?
//
