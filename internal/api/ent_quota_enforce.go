...
// ErrTenantQuotaExceeded 表示租户当期 token 配额已耗尽（EnforceTenantQuota 唯一�?nil 返回）�?
var ErrTenantQuotaExceeded = errors.New("tenant quota exceeded")

// quotaRedis 是配额计数所需的最�?Redis 接口（db.RedisClient 满足；测试可注入 fake）�?
...
// quotaIncrScript 原子自增并刷新月�?日末过期时间，返回自增后的值�?
...
//
// 语义�?
//   - 租户�?token 配额池配�?�?直接放行（nil）�?
//   - Redis 计数�?ent:quota:tokens:{tenantID}:{yyyymm|yyyymmdd} INCR（Lua 原子自增 +
//     周期末过期）；缓存键缺失时先�?billing_records SQL 聚合回填�?
//   - 任何策略/存储查询失败一�?fail-open（返�?nil + slog.Warn），
//     保证现有聊天链路永不因配额子系统中断�?
//
// 接线点（集成任务）：gateway_router.go registerAgentRoutes �?POST /submit
...
func (e *tenantQuotaEnforcer) enforce(ctx context.Context, claims *auth.Claims, estimatedTokens int64) error {
	...
	if estimatedTokens <= 0 {
		estimatedTokens = 1 // 无预估时最小增量，保持计数器热�?
	}
...
// tenantTokenUsage 读取租户当期 token 用量：优�?Redis 计数器，缺失�?
