...
// ── 短信验证码登�?+ 手机号绑�?─────────────────────────
//
// 防滥用四保险（复用既有设施）�?//  1. 人机验证栅栏：发�?登录均过 CaptchaHandler.Enforce（启�?失败升级强制）；
...
const (
	...
	smsCodeKeyPrefix  = "sms:code:"      // 验证�?	smsTriesKeyPrefix = "sms:tries:"     // 尝试计数
	smsCoolKeyPrefix  = "sms:cool:"      // 发送冷却标�?	smsDailyKeyPrefix = "sms:day:"       // 每日发送计�?	smsCodeDigits     = 6                // 验证码位�?	smsDailyWindow    = 24 * time.Hour   // 每日计数窗口
	smsMaxDailyLimit  = 100              // 每日上限配置上限
	smsMaxCodeTTL     = 15 * time.Minute // 验证码有效期上限
)

// smsCodeStore 抽象验证码存取（生产 Redis，测试内�?fake）�?type smsCodeStore interface {
...
// redisSmsCodeStore �?Redis 实现�?type redisSmsCodeStore struct {
	rdb db.RedisClient
}
...
// smsConfigRow �?ent_sms_config 的内存形态（secret 保留密文）�?type smsConfigRow struct {
	Provider         string
	SignName         string
	TemplateID       string
	AccessKeyID      string
...
// SmsHandler 提供短信验证码登录、手机号绑定/解绑与短信服务配置管理�?type SmsHandler struct {
	auth    *auth.Authenticator
	cfg     *config.Config
	db      entQuerier
	encKey  []byte
	sender  auth.SmsSender
	captcha *CaptchaHandler // 可选：nil 跳过人机验证（单测用�?	store   smsCodeStore
}

// NewSmsHandler 构造短�?handler；加密密钥沿�?SSO 密钥，验证码存储依赖 Redis�?func NewSmsHandler(authenticator *auth.Authenticator, cfg *config.Config, captcha *CaptchaHandler) *SmsHandler {
	return &SmsHandler{
		auth:    authenticator,
		cfg:     cfg,
		db:      pgEntStore{},
...
// RegisterPublicRoutes 挂载公开路由（无 authMW；外层须�?rlMW）�?func (h *SmsHandler) RegisterPublicRoutes(mux *http.ServeMux, rlMW func(http.Handler) http.Handler) {
...
// RegisterUserRoutes 挂载用户自助路由（authMW）：手机号查�?/ 绑定 / 解绑�?func (h *SmsHandler) RegisterUserRoutes(mux *http.ServeMux, authMW func(http.Handler) http.Handler) {
...
// RegisterAdminRoutes 挂载管理路由（authMW + RequireEntPerm("sso:manage")）�?func (h *SmsHandler) RegisterAdminRoutes(mux *http.ServeMux, authMW func(http.Handler) http.Handler) {
...
// 前端据此决定是否展示"短信登录"标签页�?func (h *SmsHandler) PublicStatus(w http.ResponseWriter, r *http.Request) {
...
// SendCode POST /v1/auth/sms/code（公开，须�?rlMW�?// 防滥用：人机验证 + 发送冷�?+ 每日上限�?func (h *SmsHandler) SendCode(w http.ResponseWriter, r *http.Request) {
...
// Login POST /v1/auth/sms/login（公开，须�?rlMW�?func (h *SmsHandler) Login(w http.ResponseWriter, r *http.Request) {
...
// provisionSmsUser 自动建号：email 以手机号兜底、随机不可登录密码（password_set=FALSE）�?func (h *SmsHandler) provisionSmsUser(ctx context.Context, phone string) (UserResponse, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		// 并发建号：另一请求已用该手机号建号，直接复�?		err = h.db.QueryRow(ctx,
			`SELECT id, email, name, role FROM users WHERE tenant_id = $1 AND phone = $2`,
			db.DefaultTenantID, phone).Scan(&user.ID, &user.Email, &user.Name, &user.Role)
	}
	if err != nil {
...
// verifyCode 校验验证码；失败时已写响应并返回 false�?// 错误累计 smsMaxTries 次后作废验证码，并计�?IP 失败计数（触发人机验证升级）�?func (h *SmsHandler) verifyCode(w http.ResponseWriter, r *http.Request, phone, code string) bool {
	ctx := r.Context()
...
// GetBind GET /v1/auth/sms/bind（authMW）返回当前绑定手机号�?func (h *SmsHandler) GetBind(w http.ResponseWriter, r *http.Request) {
...
// Bind POST /v1/auth/sms/bind（authMW）验证码校验后绑定手机号�?func (h *SmsHandler) Bind(w http.ResponseWriter, r *http.Request) {
	claims := auth.GetClaims(r.Context())
	if claims == nil {
		Unauthorized(w, ErrAuthRequired)
		return
...
// Unbind DELETE /v1/auth/sms/bind（authMW�?// 守卫：无口令密码且无三方身份时拒绝解绑（保留至少一种登录方式）�?func (h *SmsHandler) Unbind(w http.ResponseWriter, r *http.Request) {
	claims := auth.GetClaims(r.Context())
	if claims == nil {
		Unauthorized(w, ErrAuthRequired)
		return
...
// GetConfig GET /v1/ent/sms/config（secret 脱敏）�?func (h *SmsHandler) GetConfig(w http.ResponseWriter, r *http.Request) {
	row, err := h.loadConfig(r.Context())
	if err != nil {
		logAndRespond(w, err, http.StatusInternalServerError, ErrDBUnavailable)
		return
...
type updateSmsConfigRequest struct {
	...
	Secret           *string `json:"secret"` // 空串/脱敏占位 = 保留原�?	Endpoint         *string `json:"endpoint"`
...
// UpdateConfig PUT /v1/ent/sms/config（单租户单行 upsert）�?func (h *SmsHandler) UpdateConfig(w http.ResponseWriter, r *http.Request) {
	...
	// 以既有值（或缺省）为底，逐字段覆�?	row := &smsConfigRow{
		Provider: auth.SmsAliyun, CodeTTLSeconds: 300, SendIntervalSecs: 60, DailyLimit: 10,
	}
	if existing != nil {
		*row = *existing
	...
	// secret：新值加密；空串/占位保留原密�?	secretEnc := ""
	if existing != nil {
	...
	// 数值边�?	if row.CodeTTLSeconds < 60 || row.CodeTTLSeconds > int(smsMaxCodeTTL.Seconds()) {
	...
	// 启用前置校验（fail-loud�?	if row.Enabled {
		if secretEnc == "" {
