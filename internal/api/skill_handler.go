...
// RegisterRoutes 注册技能路由。安全修复（P0-S2）：所有路由必须经�?authMW
// （技能即代码，未认证�?install/run = 未认�?RCE），并挂 rlMW �?sanitizeMW�?
func (h *SkillHandler) RegisterRoutes(mux *http.ServeMux, authMW, rlMW, sanitizeMW routeMiddleware) {
	// 同时注册精确和尾斜杠变体：Go ServeMux �?"/v1/skills/" 只匹配子树（不匹�?"/v1/skills"），
	// 而前端调用的�?GET /v1/skills，Python 端注册的也是 /v1/skills�?
	...
	// 市场技能注册（P1 修复：前�?SkillMarketCard 调用 /v1/skills/{id}/register�?
	// 此前两端均无此路由导致注�?404�?
...
func (h *SkillHandler) proxy(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case "GET":
		// 规范化转发路径：Python 端注册的�?/v1/skills（无尾斜杠）
	...
	case "POST", "PUT":
		...
		// 身份注入（网关为唯一可信边界）：引擎无鉴权，必须由网关注入租�?用户身份�?
...
// skillTenantID 取当前租�?ID：claims 优先，缺省回退默认租户�?
...
// filterDiscoverByMarket �?discover 代理响应做市场白名单过滤�?
