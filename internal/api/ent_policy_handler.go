...
// ErrModelNotAllowed 表示请求的模型不在租�?角色白名单内�?03 语义）�?
// 集成任务（如 /submit 拦截）应将该错误映射�?HTTP 403�?
var ErrModelNotAllowed = errors.New("model not allowed by enterprise policy")

// EntTenantPolicy 对应 ent_tenant_policies 表（租户级隐�?留存策略）�?
...
// EntPolicyHandler 提供企业合规策略 API（租户隐私策�?/ 模型访问策略 CRUD），
...
//
// 解析规则：role 精确策略优先（用户直接角�?�?群组成员角色的任一命中），
// 缺失则回退租户级策略（role_id IS NULL）；两者都�?�?放行�?
// 任何查询失败一�?fail-open（放�?+ slog.Warn），保证策略基础设施故障
// 不阻断业务链路。model 不在白名单返�?ErrModelNotAllowed�?03 语义）�?
...
// 查询失败返回 error（供 GET /v1/models 改造后�?fail-open 处理）�?
...
// GetTenantPrivacyMode 返回租户是否开启隐私模式（无记�?�?false）�?
// 集成任务据此在转�?Python 引擎请求时注�?X-Privacy-Mode: no_retention 头�?
...
// modelRateLua 单键窗口限流原子脚本：超限返�?0，否则计�?+1 并返�?1�?
const modelRateLua = `
...
// CheckModelRate 每模型每租户速率检查（RPM�?0s 窗口）�?
// 限速值优先取 ent_model_policies.per_model_limits[model]["rpm"]（租户级策略），
// 缺省回退入参 rpmLimit（调用方�?admin_model_configs.max_rpm 读取）�?
// 键格�?model:{model}:{tenant}；limit<=0 �?Redis 不可�?出错时放行（fail-open）�?
...
// defaultResolveAllowedModels �?PG 解析用户适用的模型白名单�?
// 步骤：tenantID 缺省�?users 表解�?�?聚合用户角色（直�?�?群组）→
...
// policyModelRPM 读取租户级策�?per_model_limits[model]["rpm"]（无 �?0）�?
...
// validModelIDSet �?admin_model_configs 读取合法 model_id 集合（白名单校验用）�?
...
// validateAllowedModels 校验白名单元素均存在�?admin_model_configs.model_id�?
// 返回非法元素列表（空 = 合法）�?
...
// allowed_models 元素必须存在�?admin_model_configs.model_id（违�?422）；
...
// UpdateModelPolicy PUT /v1/ent/model-policies/{id}（全量替�?allowed_models /
