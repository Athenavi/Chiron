...
// maxKBDocSize 限制 kb_doc 文档大小（防 finalizeKBDoc 整文件读入内存导�?OOM）�?
...
// claimsOf 返回当前请求�?claims（含 tenant_id �?user_id）�?
...
func (h *UploadHandler) Init(w http.ResponseWriter, r *http.Request) {
	...
	var body struct {
		...
		ParentID  string `json:"parent_id"` // media 文件�?id；kb_doc 时传 kb_id
	...
	// P0-S3 路径穿越修复：文件名必须净化，拒绝 / \ .. �?
	body.Name = sanitizeUploadName(body.Name)
	if body.Name == "" {
		BadRequest(w, "invalid file name")
		return
	}
	// P0 存储�?XSS 防护（分片路径补齐）：可执行/脚本 MIME �?html/xml 类扩展名一律拒�?
	// （直传路径已�?isExecutableMIME；此处覆盖分片上传，避免 .html 被按 text/html 同源输出�?
	...
	// P0-P3 防护：kb_doc 会整文件读入内存（知识文�?content 列），限制大�?
...
func (h *UploadHandler) PutChunk(w http.ResponseWriter, r *http.Request) {
	...
	// 限制单分片大小，防止恶意客户端发送超�?chunk 撑爆磁盘（P1-1�?
...
// mergeChunks �?index 顺序拼接分片为单个临时文件�?
...
// finalizeMedia 合并文件写入 media 存储区并�?media_assets（按当前租户）�?
...
// finalizeKBDoc 合并文件内容�?knowledge_documents（content bytea �?RAG 构建，按当前租户）�?
