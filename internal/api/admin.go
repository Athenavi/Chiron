...
func (h *AdminHandler) RegisterRoutes(r *http.ServeMux) {
	...
	// ÐÂÔö¶Ëµã£ºAPI Key ¹ÜÀí
	r.HandleFunc("GET /api-keys", h.ListApiKeys)
	// ÔËÎ¬Àà¶Ëµã£¨×â»§/ÓòÃû/Êý¾Ý¿â/Redis/Ä£ÐÍ/¶¨Ê±ÈÎÎñ£©¡ª¡ª /admin È«Õ»Êµ×°
...
func (h *AdminHandler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	// S °²È«ÐÞ¸´£º·Ç owner ²»¿É½«½ÇÉ«ÌáÉýÎª owner£¨·ÀÖ¹ admin ÌáÈ¨£©
	...
	// Build dynamic UPDATE with column name whitelist ¡ª tenant_id ×÷Îª¶îÍâ WHERE Ìõ¼þ·ÀÔ½È¨
	// S °²È«ÐÞ¸´£ºÁÐÃû±ØÐëÀ´×Ô°×Ãûµ¥£¬·ÀÖ¹ SQL ×¢Èë
...
func (h *AdminHandler) CreateBackup(w http.ResponseWriter, r *http.Request) {
	// P0-P4 ÐÞ¸´£ºpg_dump Êä³öÁ÷Ê½×ª·¢£¬±ÜÃâÕû¿â»º³åÈëÄÚ´æµ¼ÖÂ OOM
...
func (h *AdminHandler) RestoreBackup(w http.ResponseWriter, r *http.Request) {
	...
	defer file.Close()
	// P0-P4 ·À»¤£ºÏÞÖÆ»Ö¸´ÎÄ¼þ´óÐ¡£¬±ÜÃâÕûÎÄ¼þ¶ÁÈëÄÚ´æ
...
func (h *AdminHandler) UpdateStorage(w http.ResponseWriter, r *http.Request) {
	...
	if previous != body.Backend {
		if previous == "local" {
			warning = "´æ´¢ºó¶ËÒÑ´Ó local ÇÐ»»Îª s3¡£¾Éºó¶ËÖÐµÄÎÄ¼þ²»»á×Ô¶¯Ç¨ÒÆ¡£"
		} else {
			warning = "´æ´¢ºó¶ËÒÑ´Ó s3 ÇÐ»»Îª local¡£¾Éºó¶ËÖÐµÄÎÄ¼þ²»»á×Ô¶¯Ç¨ÒÆ¡£"
		}
	}
