"""
SQLAlchemy 模型定义 - AuditLog
由代码生成器自动生成 (基于 models.yaml / routes.yaml) - 请勿手动修改
生成时间：2026-08-26 12:50:58
"""

from sqlalchemy import Column, Integer, BigInteger, String, Text, Boolean, DateTime, ForeignKey
import uuid

from . import Base  # 使用统一的 Base



class AuditLog(Base):
    """审计日志模型"""
    __tablename__ = 'audit_logs'




    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), doc='日志 ID')

    tenant_id = Column(String(36), ForeignKey('tenants.id'), doc='租户 ID')


    user_id = Column(String(36), ForeignKey('users.id'), nullable=True, doc='用户 ID')


    action = Column(String(64), nullable=True, doc='操作类型')

    resource_type = Column(String(64), nullable=True, doc='资源类型')

    resource_id = Column(String(64), nullable=True, doc='资源 ID')

    details = Column(String(255), default='{}', doc='详细信息（JSONB）')

    ip_address = Column(String(45), nullable=True, doc='IP 地址')

    created_at = Column(String(255), default='now()', doc='创建时间')


    def to_dict(self, exclude_sensitive=True):
        """转换为字典

        Args:
            exclude_sensitive: 是否排除敏感字段（密码、密钥、token 等）
        """
        data = {
            'id': self.id,
            'tenant_id': self.tenant_id,
            'user_id': self.user_id,
            'action': self.action,
            'resource_type': self.resource_type,
            'resource_id': self.resource_id,
            'details': self.details,
            'ip_address': self.ip_address,
            'created_at': self.created_at,
        }

        if not exclude_sensitive:
            sensitive_data = {
            }
            data.update(sensitive_data)

        return data

    def __repr__(self):
        """字符串表示"""
        return f'<AuditLog id={self.id}>'


