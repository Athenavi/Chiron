...
async function fetchStatus() {
  ...
  try {
    ...
    if (!s.needed) {
      // 先检查 install.lock 是否实际完成，避免数据库残留无 owner 记录导致误判
      try {
      ...
      // PostgreSQL 不可达 → 安装模式，走三步骤向导（需要安装令牌）
      await loadWizard()
    } else {
    ...
    error.value = true
    message.error(t('无法连接后端服务，请确认服务已启动'))
  } finally {
    loading.value = false
  }
}
...
async function loadWizard() {
  ...
  try {
    ...
    if (e?.response?.status === 401) {
      ...
      error.value = true
      message.error(e?.response?.data?.error || t('无法读取安装状态'))
    }
  }
}
...
async function submitToken() {
  ...
  if (!tok) {
    message.warning(t('请输入安装令牌'))
    return
  }
  // 保存到 localStorage，后续页面刷新/导航无需 URL 参数
  saveInstallToken(tok)
  message.success(t('令牌已保存，正在加载安装向导...'))
  await loadWizard()
}
...
async function skipStep1() {
  ...
  if (!dbForm.value.app_secret.trim()) {
    message.warning(t('请输入 APP_SECRET（部署主密钥，至少 32 字符）'))
    return
  }
  if (dbForm.value.app_secret.trim().length < 32) {
    message.warning(t('APP_SECRET 长度不足，请使用至少 32 字符的随机字符串'))
    return
  }
  // 将 APP_SECRET 保留在 dbForm 中，提交 Step 2 时一并发送
  appSecretSet.value = true
  wizard.value = 'db'
  message.success(t('APP_SECRET 已设置，请继续配置数据库'))
}
...
async function submitStep2() {
  ...
  if (!dbForm.value.postgres_dsn.trim()) {
    message.warning(t('PostgreSQL 连接串（DSN）必填'))
    return
  }
  ...
  try {
    ...
    wizard.value = 'admin'
    message.success(t('数据库配置已保存并验证通过'))
  } catch (e: any) {
    message.error(e?.response?.data?.error || t('数据库配置失败'))
  } finally {
...
async function submitStep3() {
  ...
  try {
    ...
    wizard.value = 'done'
    message.success(t('安装完成'))
  } catch (e: any) {
    const errMsg = e?.response?.data?.error || ''
    if (errMsg.includes('数据库连接已失效') || errMsg.includes('请重新完成数据库配置')) {
      message.warning(t('安装中断后数据库连接已失效，请重新配置数据库'))
      wizard.value = 'db'
    } else {
      message.error(errMsg || t('创建管理员失败'))
    }
  } finally {
    submitting.value = false
  }
...
function goBackToEnv() {
  wizard.value = 'env'
  message.info(t('返回环境检测步骤'))
}

function goBackToDb() {
  wizard.value = 'db'
  message.info(t('返回数据库配置步骤'))
}
...
async function submitLegacy() {
  ...
  try {
    ...
    message.success(t('初始化成功，请登录'))
    router.replace('/login')
  } catch (e: any) {
    message.error(e?.response?.data?.error || t('初始化失败'))
  } finally {
    submitting.value = false
  }
}
  ...
  <div class="install-page">
    <div class="install-card">
      ...
      <a-spin :spinning="loading">
        ...
        >
          {{ $t('本部署仅需在') }} <code>.env</code> {{ $t('配置') }} <b>APP_SECRET</b>（唯一主密钥）、PostgreSQL / Redis / CORS / 存储 / 模型 / 支付等配置。
        ...
        <template v-else-if="wizard === 'token'">
          <p class="install-hint hint-warn">
            {{ $t('当前处于') }} <b>{{ $t('安装模式') }}</b>{{ $t('（系统未配置数据库/主密钥）。安装页面受令牌保护：') }}
          </p>
          <ol class="token-steps">
            <li>{{ $t('查看服务启动日志中的') }} <code>install_url</code>{{ $t('（形如') }} <code>/install?token=xxx</code>）；</li>
            <li>{{ $t('使用日志中的完整地址（含令牌）重新访问本页面，或') }} <b>{{ $t('在下方输入令牌') }}</b>。</li>
          </ol>
          <p class="install-hint hint-info">
            {{ $t('提示：未配置 APP_SECRET 时令牌为随机生成（重启后变化）；配置 APP_SECRET 后令牌由其确定性派生。') }}
          </p>
          <a-form
            layout="vertical"
            @finish="submitToken"
          >
            <a-form-item :label="$t('安装令牌')">
              <a-input
                v-model:value="manualToken"
                :placeholder="$t('在此粘贴启动日志中的 token')"
              />
            </a-form-item>
            <a-button
              type="primary"
        ...
        <template v-else-if="wizard === 'env'">
          ...
          >
            <a-step :title="$t('环境检测')" />
            <a-step :title="$t('数据库配置')" />
            <a-step :title="$t('创建管理员')" />
          </a-steps>
          <p class="install-hint hint-warn">
            {{ $t('系统未检测到有效的') }} <b>APP_SECRET</b>（部署级主密钥，≥32 字符）。它是 JWT / 配置加密的唯一密钥来源。
            您可以 <b>{{ $t('在下方输入 APP_SECRET') }}</b>{{ $t('，或在') }} <code>.env</code> {{ $t('中配置后重启服务。') }}
          </p>
          <div class="env-check">
            <div class="dep-item">
              <span
            ...
            <div class="dep-item">
              ...
              >{{ dataWritable ? '✅' : '❌' }}</span>
              <span class="dep-name">{{ $t('数据目录') }}</span>
          ...
          <a-form
            ...
            @finish="skipStep1"
          >
            <a-form-item :label="$t('APP_SECRET（部署主密钥，至少 32 字符）')">
              <a-input-password
                v-model:value="dbForm.app_secret"
                :placeholder="$t('在此输入 APP_SECRET（或先在 .env 中配置后重启服务）')"
              />
            </a-form-item>
            <a-button
              type="primary"
        ...
        <template v-else-if="wizard === 'db'">
          ...
          <p class="install-hint hint-warn">
            ...
            后端将 <b>{{ $t('尝试连接验证') }}</b>{{ $t('，通过后加密保存到') }} <code>data/install.lock</code>{{ $t('；重启服务后全面生效。') }}
          </p>
          <a-form
            layout="vertical"
            @finish="submitStep2"
          >
            ...
            <a-form-item :label="$t('Redis 地址（选填）')">
              <a-input
                v-model:value="dbForm.redis_addr"
                placeholder="localhost:6379"
              />
            </a-form-item>
            <a-form-item :label="$t('Redis 密码（选填）')">
              <a-input-password
                v-model:value="dbForm.redis_password"
                :placeholder="$t('无密码可留空')"
              />
            </a-form-item>
            <a-form-item :label="$t('Redis DB（选填）')">
              <a-input-number
        ...
        <template v-else-if="wizard === 'done'">
          <div class="installed-state">
            ...
            <p class="installed-desc">
              {{ $t('管理员账户已创建，数据库配置已保存。请') }} <b>{{ $t('重启服务') }}</b> {{ $t('使全部功能生效，然后使用管理员凭据登录。') }}
            </p>
            <a-button
              type="primary"
              size="large"
        ...
        <template v-else-if="wizard === 'legacy'">
          <p class="install-hint hint-warn">
            ...
            <template v-else>
              {{ $t('PostgreSQL 连接正常，但 Redis 不可用（服务以降级模式运行）。仍可直接创建管理员账户完成初始化。') }}
            </template>
          </p>
          <a-form
            layout="vertical"
