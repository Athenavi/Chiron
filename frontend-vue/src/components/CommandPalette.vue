 ...
 * 类似 Linear / VS Code 的全局命令入口
 *  - 静态动作：切换六大工作台、切换主题、打开快速命令（/chat?task=）、查看市场、新建会话
 *  - 远程搜索：GET /v1/search?q=xxx（防抖300ms，输入≥2字符触发）→ 消息 / 媒体结果
...
const workstationActions: PaletteEntry[] = [
  ...
  { id: 'ws_workflow', group: '工作台', label: '工作台', desc: 'DAG 流程编排', keywords: 'workflow 工作台 dag 流程 编排', icon: markRaw(ApartmentOutlined), run: () => void router.push('/workflow') },
  { id: 'ws_skills', group: '工作台', label: '技能', desc: '工具 MCP', keywords: 'skill 技能 mcp 工具', icon: markRaw(ThunderboltOutlined), run: () => void router.push('/skills') },
  { id: 'ws_knowledge', group: '工作台', label: '知识', desc: 'RAG 检索增强', keywords: 'knowledge 知识 rag 检索 文档', icon: markRaw(BookOutlined), run: () => void router.push('/knowledge') },
...
// 全局 Ctrl/Cmd+K（preventDefault 屏蔽浏览器行为）；打开后 Esc 也可关闭
        ...
        >
          ...
          <div ref="listEl" class="palette-body">
            ...
            <EmptyState
              ...
              size="list"
              description="未找到匹配结果"
              hint="试试更短的关键词，或直接用 Enter 用本地命令执行"
            />
          </div>
